 ...
 * وعوامل لا وجود لها في لواو (مثل +=). النصوص الحرفية والتعليقات العربية مسموحة.
 *
 * الوعي باللغة ضروري: عارض `#` طولٌ في لواو لكنه تعليق في بايثون، و`//` و`+=`
 * صالحان في جافاسكريبت/C++ بينما ممنوعان في لواو. لذلك الفحص والتنظيف يعملان
 * باصطياد تعليقات اللغة المستهدفة فقط.
 */
...
export function hasInvalidCode(code: string, lang: CodeLang = "lua"): string | null {
  ...
  // عوامل غير مدعومة في لواو فقط: += -= *= /=
  if (lang === "lua") {
    const badAssignee = stripped.match(/[+\-*/]=[^=]/);
    if (badAssignee) return "عامل اختصار غير مدعوم (=+ وما شابه) في الكود";
  }
...
/**
 * يعيد تسمية المعرّفات غير اللاتينية (عربية) بأسماء لاتينية آمنة ويصلح
 * عوامل لواو الممنوعة (+= ← = +) — مع الحفاظ على السلاسل والتعليقات حسب لغة
 * الدرس. الاستبدال متسق داخل الدرس: نفس الاسم العربي → نفس الاسم اللاتيني دوماً.
 */
